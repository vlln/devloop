...
| 阶段 | 做什么 | 是否可跳过 |
|------|--------|-----------|
| INIT | 搭建项目骨架（目录、标准文件、Git） | 首次必选 |
| DESIGN | 冻结四类契约（业务/接口/质量/架构），拆解 Plan | 不可跳过 |
| DEVELOP | 多轨道并行：编码/测试基建/部署 | 不可跳过 |
| **INTEGRATE** | **全量测试。唯一串行关口——必须等所有轨道完成。不可跳过** | **不可跳过** |
| PRE_RELEASE | 预发布环境验证、版本号确认 | 不可跳过 |
| RELEASE | 版本归档、CHANGELOG 整理、迭代复盘 | 不可跳过 |
