...
编写用户故事、模块划分、数据模型、接口定义、业务规则、UI 约束。参考 `docs/spec/001-template.md`。模块划分表需包含「目录路径」列（模块→源码目录的映射）。
...
每条 AC 必须覆盖四个场景维度（正常/边界/异常/失败）。只写正常流程的 AC 属于无效 AC。参考 `docs/ac/001-template.md`。
...
**验证方式：** 创建临时 branch，编写最小验证代码，验证后丢弃。结果记录在 ADR 的「验证」段。

**退出条件：** 所有需验证的 ADR 验证段非空且结论为「可行」。不可行则退回修改 ADR，重新验证。

### 接口定义（适用：有 API 的项目）

**产出：** 接口定义文档（独立于 Spec，参考 `docs/interface/001-template.md`）
...
| 模块划分 ≥ 1 个，每个含职责说明 + 目录路径；单模块需说明原因 | 读模块划分表，检查无空列 |
