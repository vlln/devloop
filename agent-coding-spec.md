...
| TEST_INFRA | 搭建测试基建、创建执行单元、更新 CONTRIBUTING.md 测试段 | 编写业务测试用例、编写业务代码 |
...
| 维度 | 定义 |
|------|------|
| **含义** | 基于 TEST_INFRA 搭建的一次性基建，按 Spec 模块划分创建执行单元，TDD 左移开发 |
...
**热修复：** 阻断性故障走快速通道（RELEASE → DESIGN 轻量 → TEST_INFRA 增量 → DEVELOP 最小修复 → SYSTEM_TEST 回归 → RELEASE 补丁 tag）。
...
| 6 阶段状态机 | INIT→DESIGN→TEST_INFRA→DEVELOP→SYSTEM_TEST→RELEASE。测试开发与业务开发分离 |
