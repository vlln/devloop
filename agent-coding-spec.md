...
| TEST | 搭建测试基建、创建执行单元、更新 CONTRIBUTING.md 测试段 | 编写业务测试用例、编写业务代码 |
...
| 测试基建 ADR | 测试框架、Mock、CI 平台 | TEST 阶段 |
...
- **契约是并行化的唯一前提。** 一旦 DESIGN 出口把关通过（Spec + ADR + 接口定义 + AC 全部冻结），TEST 和 DEVELOP 内可并行。
...
```
INIT → DESIGN → TEST → DEVELOP → SYSTEM_TEST → RELEASE
        ↑         ↑        ↑            ↑
        │         │        │            │
        └─────────┴────────┴────────────┘
        (架构颠覆 / 基建缺陷 / 设计缺陷 回退)
...
| 维度 | 定义 |
|------|------|
| **含义** | 项目骨架搭建。将项目接入 devpact 系统。新项目安装模板，旧项目扫描合并 |
...
| **禁止** | 编写业务代码 |
| **准入条件** | INIT 完成 |
| **→ TEST 条件** | 全部文档 status=proposed，出口把关内容级审查通过，promote proposed→active/accepted |
...
| **允许** | 编写测试基建 ADR、创建执行单元、搭建基建、更新 CONTRIBUTING.md 测试段 |
| **禁止** | 编写业务测试用例、编写业务代码 |
| **准入条件** | DESIGN 完成（契约冻结） |
| **→ DEVELOP 条件** | CI 可运行、MR 门禁正确拦截、提测门禁正确拦截、Mock 返回正确、覆盖率数据准确、E2E 框架可跑冒烟 `[适用]`、E2E 脚本对齐 AC `[适用]`、CONTRIBUTING.md 测试段已填写 |
...
**设计理由：** 测试开发与业务开发分离。TEST 阶段搭建武器工厂，DEVELOP 阶段制造武器。测试基建自证能正确判断对错（不仅能启动），而非在业务开发中边用边修。

#### DEVELOP — 编码开发（最高并行期）

| 维度 | 定义 |
|------|------|
| **含义** | 基于 TEST 搭建的一次性基建，按 Spec 模块划分创建执行单元，TDD 左移开发 |
| **允许** | 创建 Plan、执行 Plan（编码/测试）、更新 Plan/Report |
| **禁止** | 修改 Spec/ADR/AC 文档、修改全局契约 |
| **准入条件** | TEST 完成（一次性基建就绪） |
| **→ SYSTEM_TEST 条件** | 全部 Plan 完成、MR 门禁全部通过、打包部署成功、提测门禁通过 |

**DEVELOP 子阶段：**

| 子阶段 | 适用 | 产出 |
|--------|------|------|
| 创建 Plan | 必选 | Spec 模块划分表 → Plan 文件夹 |
...
#### SYSTEM_TEST — 系统测试（唯一串行关口）

| 维度 | 定义 |
|------|------|
| **含义** | 执行系统级验证——集成测试、系统测试、专项测试。单元测试和开发集成自测已在 DEVELOP 完成 |
| **允许** | 执行全量测试、修复 bug、生成测试报告 |
| **禁止** | 新增功能代码、修改 Spec/ADR |
| **准入条件** | DEVELOP 全部 Plan done |
| **→ RELEASE 条件** | 全部测试层通过，无阻塞级缺陷 |
| **回退** | 基建缺陷 → TEST，设计缺陷 → DESIGN |
...
**修复循环：** 修复在 SYSTEM_TEST 内部完成。局部 bug 内部修复；基建缺陷 → TEST；设计缺陷 → DESIGN。

#### RELEASE — 迭代闭环

| 维度 | 定义 |
|------|------|
| **含义** | staging 验证 → production 发布 → 归档。PRE_RELEASE 作为内部步骤，不再独立阶段 |
| **允许** | CD 部署、冒烟测试、打 tag、整理 CHANGELOG、归档 |
| **禁止** | 修改任何代码 |
| **准入条件** | SYSTEM_TEST 全部测试通过 |
| **→ DESIGN 条件** | 新一轮迭代或热修复 |

**RELEASE 子阶段：** staging 验证（部署 + 冒烟 + 环境检查）→ production 发布（发布策略确认 + 监控 + 回滚方案 + 打 tag + 归档）。

**staging 失败分类：** CD/环境失败 → 修复后重新部署；业务 bug → 退回 DEVELOP。

**热修复：** 阻断性故障走快速通道（RELEASE → DESIGN 轻量 → TEST 增量 → DEVELOP 最小修复 → SYSTEM_TEST 回归 → RELEASE 补丁 tag）。
...
| **创建时机** | DESIGN 阶段（业务 ADR）+ TEST 阶段（测试基建 ADR） |
...
| 维度 | 定义 |
|------|------|
| **定位** | 执行单元。任何需要执行的任务都通过创建 Plan 文件夹来组织 |
| **创建时机** | 各阶段自行创建（TEST 创建基建 Plan，DEVELOP 创建业务 Plan） |
...
| 目录 | README 内容 |
|------|------------|
| `docs/` | 子目录列表、**当前阶段**、**最近事件** |
| `docs/spec/` | Spec 版本列表（文件、状态） |
| `docs/interface/` | 接口定义列表 |
| `docs/ac/` | AC 文档列表 |
| `docs/adr/` | 决策列表（编号、标题、状态） |
| `docs/plans/` | 任务列表（编号、标题、状态、创建时间） |
| Plan 文件夹内 | 子任务状态表（编号、子任务、Plan、Report、覆盖 AC、状态、Commits） |

**docs/README.md 关键字段：**

- **当前阶段**：INIT/DESIGN/TEST/DEVELOP/SYSTEM_TEST/RELEASE
...
- 测试基础设施（CI/Mock/E2E/覆盖率/门禁）由 TEST 阶段一次性搭建，自证正确性
...
|------|------|
| 验收标准 | AC 文档 |
| 测试基建 ADR | docs/adr/（TEST 阶段创建） |
| 测试命令 | CONTRIBUTING.md（TEST 阶段填写） |
| 测试目录 | CONTRIBUTING.md（TEST 阶段填写） |
...
| 最近事件 | 状态变更时追加，Agent 中断恢复时重建上下文 |
| Git 作为存储底座 | 状态流转以 commit 为切分边界 |
