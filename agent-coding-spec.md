...
```
INIT ──→ DESIGN ──→ PLAN_READY ──→ DEVELOP ──→ INTEGRATE ──→ PRE_RELEASE ──→ RELEASE
            ↑            ↑            │  ↑          │                           │
            │            │            │  └──────────┘                           │
            │            │            ▼              │                           │
            │            │      CHECK_PENDING ───────┘                           │
...
|------|------|
| **含义** | 项目骨架搭建阶段 |
| **允许** | 创建目录结构、创建 AGENTS.md/CONTRIBUTING.md/CHANGELOG.md、初始化 Git、初始化 CI 配置 |
...
|------|------|
| **含义** | 需求分析、架构设计、任务拆解阶段 |
| **允许** | 编写/修改 Vision、Design Spec、ADR；拆解 Plan；讨论技术方案 |
| **禁止** | 编写业务代码（src/）、执行 Plan |
| **准入条件** | INIT 完成（目录结构、全局规范就位） |
| **→ PLAN_READY 条件** | Design Spec 冻结（status=active）、核心 ADR 已采纳（status=accepted）、Plan 任务板已拆解完成 |
...
|------|------|
| **含义** | 任务板加载完毕，等待调度执行 |
| **允许** | 检查 Plan 依赖关系、确认任务就绪、分配隔离环境 |
| **禁止** | 编写业务代码、修改 Design/ADR |
| **准入条件** | DESIGN 完成（Design 冻结、ADR 采纳、Plan 拆解完成） |
| **→ DEVELOP 条件** | 所有 Plan 的依赖关系已解析、隔离环境（Git worktree）已准备、任务板无阻塞标记 |
| **触发者** | Designer |
...
| 维度 | 定义 |
|------|------|
| **含义** | Executor 在隔离环境中执行 Plan，完成编码和自测 |
| **允许** | Executor 领取任务、编写代码、单元测试、集成测试、更新 Plan/Report 状态 |
| **禁止** | 修改 Vision/Design/ADR、修改全局契约 |
| **准入条件** | PLAN_READY 完成，任务板和隔离环境就绪 |
| **→ INTEGRATE 条件** | 全部 Plan 的 done 状态达到本轮迭代阈值（由 Designer 在 Plan 拆解时设定） |
| **→ CHECK_PENDING 条件** | 单个 Plan 遇到阻塞，无法自愈（测试失败且无法自动修复） |
...
| 维度 | 定义 |
|------|------|
| **含义** | 测试不通过、存在阻塞性缺陷，等待修复 |
| **允许** | 修复 bug、补充测试、回退代码 |
| **禁止** | 新增功能、修改 Design/ADR |
| **准入条件** | DEVELOP 或 INTEGRATE 中发现阻塞性缺陷 |
| **→ DEVELOP 条件** | 修复完成，阻塞解除 |
| **→ DESIGN 条件** | 发现设计缺陷，需要回退到设计阶段 |
| **触发者** | Executor（→DEVELOP），Designer（→DESIGN） |

#### INTEGRATE — 集成校验

| 维度 | 定义 |
|------|------|
| **含义** | 跨模块集成测试、E2E 测试、全量回归，验证整体系统 |
| **允许** | 执行全量测试、修复集成问题、生成测试报告 |
| **禁止** | 新增功能代码、修改 Design/ADR |
| **准入条件** | DEVELOP 完成（全部 Plan done） |
| **→ PRE_RELEASE 条件** | 全部测试层通过（单元/集成/接口/E2E） |
| **→ CHECK_PENDING 条件** | 测试失败，存在集成缺陷 |
...
| 禁止回跳 | 除 `DEVELOP ↔ CHECK_PENDING` 内部循环和 `DESIGN` 回退外，不允许逆向流转 |
| 架构变更强制回退 | 架构方案发生颠覆性变更 → 强制退回 `DESIGN`，不允许在 DEVELOP 阶段直接修改契约 |
| 局部 bug 内部循环 | 仅局部 bug、代码实现问题 → 允许 `DEVELOP ↔ CHECK_PENDING` 内部循环，不触发全局回退 |
...
| 层面 | 使用者 | 用途 |
|------|--------|------|
| **外部调度层** | 调度/协调 Agent | 读取状态机定义，执行状态流转、准入检查、环境准备、异常上浮。**主动推进状态变更** |
| **项目文档层** | 项目内 Executor | 读取 `docs/README.md` 中的当前状态字段，**自我约束**：判断当前阶段能做什么、不能做什么 |
