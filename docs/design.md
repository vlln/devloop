...
- 冻结 = 对应文档的 `status` 字段从 `draft` 变为 `proposed`（设计师写完了），Leader 审查通过后 promote 为 `active`（或 `accepted`），伴随独立 Git commit
...
- **契约是并行化的唯一前提。** 一旦 DESIGN Leader 审查通过（Spec + ADR + 接口定义 + AC 全部冻结），TEST_INFRA 和 DEVELOP 内可并行。
...
| **禁止** | 编写业务代码 |
| **准入条件** | INIT 完成 |
| **→ TEST_INFRA 条件** | 全部文档 status=proposed，Leader 审查通过，promote proposed→active/accepted |
...
**出口把关（Leader 审查）：** 逐项审查，非全部退回。失败定位到具体文档，修复后重新检查该项。审查通过后 promote：proposed→active（ADR 为 accepted）。
...
|------|------|
| `draft` | 编写中 |
| `proposed` | 编写完成，待 Leader 审查 |
| `active` | 审查通过，当前生效 |

**设计理由：** proposed 是"设计师写完"和"系统采纳"之间的中间态。设计师标记 proposed 表示"我写完了"，Leader 审查通过后 promote 为 active。这防止未审查的内容直接生效。旧版本由 Git 历史追溯，不原地保留。
...
| 出口把关（Leader 审查）= 内容级检查 | 非形式检查，逐项审查。失败定位到具体文档 |
